...
-- Create function to check if user liked a comment
CREATE OR REPLACE FUNCTION public.is_comment_liked(comment_id UUID, user_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1 FROM public.comment_likes 
        WHERE comment_likes.comment_id = is_comment_liked.comment_id 
        AND comment_likes.user_id = is_comment_liked.user_id
    );
END;
